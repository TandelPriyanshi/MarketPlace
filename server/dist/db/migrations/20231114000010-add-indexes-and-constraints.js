...
module.exports = {
    up: async (queryInterface, Sequelize) => {
        ...
        const existingIndexes = await Promise.all(tableNames.map(tableName => queryInterface.showIndex(tableName)));
        // Add orders index if it doesn't exist
        if (!existingIndexes[0].some((index) => index.name === 'orders_status_payment_delivery_idx')) {
        ...
        if (!existingIndexes[1].some((index) => index.name === 'order_items_orderId_productId_idx')) {
        ...
        if (!existingIndexes[2].some((index) => index.name === 'visits_salesman_status_date_idx')) {
        ...
        const checkConstraintExists = async (tableName, constraintName) => {
            const [results] = await queryInterface.sequelize.query(`SELECT * FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS 
        ...
        const addCheckConstraint = async (tableName, constraintName, check) => {
    ...
    down: async (queryInterface, Sequelize) => {
        ...
        const dropCheckConstraint = async (tableName, constraintName) => {
            try {
                await queryInterface.sequelize.query(`
          ALTER TABLE ${tableName}
